  ...
  jQuery(document).ready(function () {
    ...
    if ($("#preloader").length > 0) {
      ...
      function hidePreloader() {
        ...
        // Добавляем класс loaded для анимации закрытия
        document.body.classList.add('loaded');
        
        // Скрываем прелоадер после анимации закрытия
        setTimeout(function(){
          $("#preloader").hide();
        }, 1000); // 1 секунда на анимацию закрытия
      }
      
      // Ждем полной загрузки всех ресурсов
      window.addEventListener('load', function(){
        // Добавляем небольшую задержку для завершения всех анимаций
        setTimeout(hidePreloader, 500); // 500ms задержка после загрузки всех ресурсов
      });
      
      // Резервный таймер на случай, если событие load не сработает
      setTimeout(hidePreloader, 3000); // Максимум 3 секунды
    }
    /**
     * ======================================
     * 04. data background
     ...
     * ======================================
     */
    if ($(".mouseCursor").length > 0 && window.PerformanceManager) {
      // Ждем инициализации PerformanceManager
      window.PerformanceManager.init().then(() => {
        const performanceConfig = window.PerformanceManager.getConfig();

        if (performanceConfig.cursor.enabled) {
          function itCursor() {
            ...
            if (myCursor.length) {
              if ($("body")) {
                ...
                // Throttle function для оптимизации
                ...
                const throttledMouseMove = throttle(function (s) {
                  o ||
                    (t.style.transform =
                    ...
                    (n = s.clientY),
                    (i = s.clientX);
                }, performanceConfig.cursor.throttle);
          ...
          // Отключаем курсор для низкой производительности
          $(".mouseCursor").hide();
        }
      });
    }
    ...
    if (device_width >= 576) {
      ...
      caseStudyItem.forEach((item, i) => {
        const throttledMove = throttle((event) => {
          requestAnimationFrame(() => followImageCursor(event, item));
        }, 16); // ~60fps
